...
export const content: SectionContent = {
  ...
  description:
    'The Okta MCP Adapter is a self-hosted OAuth 2.1 authorization gateway for MCP — a 7-layer proxy pipeline that brokers token exchange between AI agents and backend MCP servers, with 8 auth methods, per-agent ACLs, and full audit trail.',
  ...
  cards: [
    {
      heading: 'What the MCP Adapter Actually Is',
      paragraphs: [
        'The Okta MCP Adapter is a self-hosted OAuth 2.1 authorization gateway that sits between AI coding agents (Claude Code, Cursor, GitHub Copilot, Glean, Devin) and backend MCP servers. It solves a specific problem: third-party AI agents cannot participate in Okta Cross App Access (XAA) flows because they do not integrate with enterprise IdPs. They bypass the enterprise identity layer entirely. The Adapter intercepts this gap — it authenticates agents via Okta, brokers token exchange with backend resource servers, enforces per-agent access control lists, and logs every tool call as a structured audit event.',
        'The core value proposition in one sentence: "Login once via Okta, call many backend MCP servers — with per-agent access control, cross-app token exchange, and full audit trail." The Adapter transforms a fragmented multi-credential problem (each agent needs separate credentials for each backend) into a single SSO flow through Okta.',
        'The Adapter is a Python/FastAPI application packaged as a Docker bundle. It consists of 5 services: the Gateway (port 8000), PostgreSQL (config, agents, audit log — AES-256-GCM encrypted credentials), Redis (token cache, session cache, pub/sub event bus), an Admin UI (Next.js on port 3001), and an observability stack (Grafana + Loki + Promtail on port 3000). It deploys on AWS EC2 with ALB + HTTPS. The current demo package is okta-mcp-aws-0.12.1.',
      ],
    },
    {
      heading: 'The 7-Layer Proxy Pipeline',
      paragraphs: [
        'Every MCP request passes through a deterministic 7-layer pipeline. Each layer has a single responsibility and can be independently tested. This is the core architecture — understanding it is essential for SE conversations about how the Adapter works.',
    ...
    },
    {
      ...
      paragraphs: [
        ...
        'SE positioning: "Okta MCP Server" answers "I want AI to manage my Okta tenant." "MCP Adapter" answers "I want to secure the MCP tool calls my agents make to any backend API." Know the difference — they serve completely different use cases and audiences.',
      ],
    },
  ],
};
